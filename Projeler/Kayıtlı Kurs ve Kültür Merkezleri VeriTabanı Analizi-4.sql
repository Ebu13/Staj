...
-- Ýl/Ýlçe Tanýmlarý Tablosu
CREATE TABLE IlIlceTanimlari (
    il_ilce_kodu NVARCHAR(6) PRIMARY KEY,
    il_ilce_aciklamasi NVARCHAR(50) NOT NULL
);

-- Mahalle Tanýmlarý Tablosu
CREATE TABLE MahalleTanimlari (
    mahalle_kodu INT PRIMARY KEY,
    mahalle_adi NVARCHAR(50) NOT NULL,
    il_ilce_kodu NVARCHAR(6) NOT NULL,
    FOREIGN KEY (il_ilce_kodu) REFERENCES IlIlceTanimlari(il_ilce_kodu)
);

-- Cadde/Sokak Tanýmlarý Tablosu
CREATE TABLE CaddeSokakTanimlari (
    cadde_sokak_kodu INT PRIMARY KEY,
    cadde_sokak_adi NVARCHAR(50),
    mahalle_kodu INT NOT NULL,
    FOREIGN KEY (mahalle_kodu) REFERENCES MahalleTanimlari(mahalle_kodu)
);
...
CREATE TABLE KursEtkinlikYerleri (
	...
	FOREIGN KEY (ust_merkez_id) REFERENCES KursEtkinlikYerleri(yer_id),
    FOREIGN KEY (cadde_sokak_kodu) REFERENCES CaddeSokakTanimlari(cadde_sokak_kodu)
);
